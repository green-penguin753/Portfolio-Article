...
「オブジェクト」を基本の単位で考え、プログラム全体をオブジェクト同士の関係として設計する手法。
...
<b>・オブジェクト</b>・・・プロパティとメソッドをまとめたもの。
<b>・クラス</b>・・・・・・オブジェクトを作る設計図。
<b>・インスタンス</b>・・・クラスを使ってオブジェクトを生成したもの。実体
<b>・プロパティ</b>・・・・クラス内で定義された変数のこと。データ・属性。
<b>・メソッド</b>・・・・・クラス内で定義された関数のこと。動作・機能。
　　　　　　　　　　データ(プロパティ)を操作するのがメソッド
...
クラスメンバとインスタンスメンバの２種類があり、「static」修飾子の有無で見分ける。
...
カプセル化とは、  
オブジェクト内にデータとそれを操作する手続きをまとめて外部から隠すこと。

- クラス内の変数をクラスの外から操作できないようにすることで、  
...
継承とは、  
親クラスで定義しているデータやメソッドを、子クラスに引き継ぐ仕組み。

- 親クラスの機能を子クラスでも利用したいときは「private」ではなく、  
  「protected」や「public」で定義しておく。
...
それにより同じメソッドを呼び出してもクラスごとに異なる動作に変更することを  
